...
class Parser(object):
			...
			if token1.type == TokenType.Separator:
				...
				while i < len(tokens):
					...
					if tokeni.type == TokenType.Keyword:
						tokeni2 = tokens[i+2]
						if tokeni.value == 'b':
							getter = getter + '\t\t' + varName + '.backgroundColor = XM_UIColorFromHEX(' + tokeni2.value + ');\n'
						elif tokeni.value == 'f':
							if 'button' in className.lower():
								getter = getter + '\t\t' + varName + '.titleLabel.font = XM_System_Font('+str(tokeni2.value)+ ');\n'
							elif 'label' in className.lower():
								getter = getter + '\t\t' + varName + '.font = XM_System_Font('+str(tokeni2.value)+ ');\n'
						elif tokeni.value == 'i':
							if 'button' in className.lower():
							...
							if 'button' in className.lower():
								getter = getter + '\t\t' + '[' + varName + ' setTitleColor:XM_UIColorFromHEX('+ tokeni2.value + ') forState:UIControlStateNormal];\n'
							else:
								getter = getter + '\t\t' + varName + '.textColor = XM_UIColorFromHEX('+ tokeni2.value + ');\n'
							...
							getter = getter + '\t\t' + varName + '.layer.borderColor = XM_UIColorFromHEX(' + tokeni2.value + ').CGColor;\n'
	...
	def viewTypeWithString(this,string):
		if 'label' in string.lower():
			...
			return 'UIScrollView'
		elif 'collectionview' in string.lower():
			return 'UICollectionView'
		elif 'slider' in string.lower():
			return 'UISlider'
		else: 
			return 'UIView'
